            ...
            <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
                <section class="rounded-[2rem] bg-slate-950 p-6 text-white shadow-2xl shadow-slate-900/20">
                    <div class="flex flex-wrap items-start justify-between gap-4">
                        ...
                        <Link :href="route('faktur-pajak.create')" class="rounded-full bg-blue-400 px-5 py-3 text-sm font-semibold text-slate-950">
                            Upload Faktur Pajak
                        </Link>
                    </div>
                ...
                <section class="mt-8 overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white shadow-sm">
                    ...
                    <div v-if="fakturPajaks.length" class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-slate-200">
                            ...
                            <tbody class="divide-y divide-slate-100">
                                <tr v-for="fakturPajak in fakturPajaks" :key="fakturPajak.id" class="hover:bg-[#fff2d9]/60">
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex flex-wrap gap-2">
                                            <Link :href="route('faktur-pajak.show', fakturPajak.id)" class="rounded-full border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-700">
                                                Detail
                                            </Link>
                                            <a v-if="fakturPajak.preview_url" :href="fakturPajak.preview_url" target="_blank" rel="noreferrer" class="rounded-full border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-700">
                                                Preview
                                            </a>
                    ...
                    <div v-else class="px-6 py-16 text-center">
                        ...
                        <Link :href="route('faktur-pajak.create')" class="mt-4 inline-flex rounded-full bg-blue-600 px-4 py-2 text-sm font-semibold text-white">
                            Upload Faktur Pajak
                        </Link>
                    </div>
                </section>
